...
MODULE_INIT(__ymodem_init, INIT_LEVEL_APP);
...
void YMODEM_SendByte(const uint8_t data)
{
    ...
    uint32_t tick)
{
    switch (ymodem->state)
    {
        case (YMODEM_STATE_IDLE):  // 空闲状态，等待接收数据
        {
            if (len > 0) // 接收到数据
            {
                ...
                while (len--)
                {
                    ymodem->pack_buf[ymodem->pack_buf_offset++] = *data++;
                }
            }

            if(get_tick_diff(tick, ymodem->tick) >= 500)
            {
                ymodem->tick = tick;
        ...
        case (YMODEM_STATE_START):  // 开始传输数据
        {
            while (len--)
            {
                ymodem->pack_buf[ymodem->pack_buf_offset++] = *data++;
            ...
            ymodem->header = ymodem->pack_buf[0];
            if (ymodem->header == YMODEM_SOH) // 128包
            {
                ymodem->state = YMODEM_STATE_RECEIVE_SOH; // 接收SOH
            }
            else if (ymodem->header == YMODEM_STX) // 1024包
            {

            }
            else if (ymodem->header == YMODEM_EOT) // 结束标志
            {
        ...
        case (YMODEM_STATE_RECEIVE_SOH):  // 接收SOH包
        {
            while (len--)
            {
                ymodem->pack_buf[ymodem->pack_buf_offset++] = *data++;
            }
            if (ymodem->pack_buf_offset == 133)
            {
                ymodem->state = YMODEM_STATE_START;
                ymodem->pack_buf_offset = 0;

                ymodem->block_number = ymodem->pack_buf[1];    // 包号
                ymodem->block_number_inv = ymodem->pack_buf[2];    // 包号反码
                                                                    // 包号验证
                ...
                if (ymodem->block_number == 0x00 && ymodem->expected_block == 0)  // 头包
                {
                    memcpy(ymodem->payload, &ymodem->pack_buf[3], 128); // 复制载荷
    ...
    case(YMODEM_SOH):       // 128字节数据包
    {
      ...
      else  // 正常数据包
      {
        ...
        if(ymodem->received_size >= ymodem->file_size)
        {
            ...
            iap_write_appbin(ymodem->flash_address, ymodem->payload, valid_len);
            ymodem->flash_address += valid_len;
        }
        else  // 正常数据包
        {
            iap_write_appbin(ymodem->flash_address, ymodem->payload, 128);
            ymodem->flash_address += 128;
        }
